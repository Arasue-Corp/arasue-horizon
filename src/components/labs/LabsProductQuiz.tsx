...
export function LabsProductQuiz({ lang }: { lang: 'en' | 'es' }) {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white rounded-[3rem] p-12 md:p-16 shadow-xl border border-[#1a2e1a]/5 overflow-hidden relative min-h-[400px]">
      {/* Decorative */}
      ...
      <AnimatePresence mode="wait">
        {step === 0 && (
          ...
          >
            <div className="w-16 h-16 bg-[#F9F7F1] rounded-full flex items-center justify-center mb-6">
              <Sparkles className="w-8 h-8 text-[#1a2e1a]" />
            </div>
            <h3 className="text-4xl font-serif text-[#1a2e1a] mb-4">{t.title}</h3>
            <p className="text-[#1a2e1a]/60 font-sans mb-10 max-w-sm mx-auto">{t.subtitle}</p>
            <button 
              onClick={() => setStep(1)}
              className="px-8 py-4 bg-[#1a2e1a] text-white rounded-full font-sans font-bold hover:scale-105 transition-all"
            >
              {t.btn_start}
            </button>
          </motion.div>
        ...
        {step === 1 && (
          ...
          >
            <span className="text-[#1a2e1a]/40 font-mono text-sm mb-4">1 / 2</span>
            <h3 className="text-3xl font-serif text-[#1a2e1a] mb-8">{t.q1}</h3>
              ...
              {t.q1_opts.map(opt => {
                ...
                return (
                  <button 
                    ...
                    className={`p-6 rounded-2xl border text-left flex flex-col gap-4 transition-all ${goal === opt.id ? 'border-[#1a2e1a] bg-[#F9F7F1]' : 'border-[#1a2e1a]/10 hover:border-[#1a2e1a]/30'}`}
                  >
                    <Icon className="w-6 h-6 text-[#1a2e1a]" />
                    <span className="font-bold text-[#1a2e1a]">{opt.label}</span>
                  </button>
                )
              })}
            </div>
        ...
        {step === 2 && (
          <motion.div 
            ...
            className="h-full flex flex-col"
          >
            <span className="text-[#1a2e1a]/40 font-mono text-sm mb-4">2 / 2</span>
            <h3 className="text-3xl font-serif text-[#1a2e1a] mb-8">{t.q2}</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 font-sans">
              {t.q2_opts.map(opt => (
                <button 
                  key={opt.id}
                  onClick={() => { setTime(opt.id); setTimeout(() => setStep(3), 300) }}
                  className={`p-6 rounded-2xl border text-center transition-all ${time === opt.id ? 'border-[#1a2e1a] bg-[#1a2e1a] text-white' : 'border-[#1a2e1a]/10 hover:border-[#1a2e1a]/30 text-[#1a2e1a]'}`}
                >
        ...
        {step === 3 && (
          ...
          >
            <p className="text-[#1a2e1a]/60 font-sans mb-2">{t.result_desc}</p>
            <h3 className="text-4xl font-serif text-[#1a2e1a] mb-6">{rec.name}</h3>
            
            <div className="flex gap-4 font-sans items-center">
              <button className="px-8 py-4 bg-[#1a2e1a] text-white rounded-full font-bold flex items-center gap-2 hover:scale-105 transition-transform">
                {rec.price} <ArrowRight className="w-4 h-4" />
              </button>
              <button onClick={() => setStep(0)} className="px-4 py-4 text-[#1a2e1a]/60 hover:text-[#1a2e1a] text-sm font-medium underline underline-offset-4 transition-colors">
                {t.btn_restart}
              </button>
            </div>
          </motion.div>
