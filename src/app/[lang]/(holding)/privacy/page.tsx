...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return {
    title: `${dict.corporate.privacy.title} | Arasue Horizon`,
    description: dict.corporate.privacy.intro
  }
}
...
export default async function PrivacyPage({
  ...
  const resolvedParams = await params
  const dict = await getDictionary(resolvedParams.lang as Locale)
  const t = dict.corporate.privacy
      ...
      <div className="max-w-4xl mx-auto w-full">
        ...
        >
          <section className="space-y-4">
            <h2 className="text-3xl font-bold font-playfair">{t.data_collection_title}</h2>
          ...
          <section className="space-y-4">
            <h2 className="text-3xl font-bold font-playfair">{t.data_usage_title}</h2>
            <p className="text-lg opacity-80 leading-relaxed">
              {t.data_usage_desc}
            </p>
          </section>
